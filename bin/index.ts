...
program
  ...
  .option(
    "-o, --output <file>",
    "📤 Output file path. Use '-' for stdout. Defaults to code.txt."
  )
  .option(
    "-s, --max-size <bytes>",
    "📐 Max file size to include (in bytes). Default: 10MB.",
    "10485760"
  )
  .option(
    "-b, --branch <name>",
    "🌿 Branch name to use when cloning a remote repository."
  )
  .option(
    "--include-pattern <patterns...>",
    "✅ Glob patterns to include specific files/folders."
  )
  .option(
    "--exclude-pattern <patterns...>",
    "❌ Glob patterns to exclude specific files/folders."
  )
  .option(
    "--include-gitignored",
    "🫥 Include files even if they are listed in .gitignore.",
    false
  )
  .option(
    "-t, --token <pat>",
  ...
  .action(async (source: string | undefined, options: CliOptions) => {
    if (!source) {
      ...
      console.log(
        chalk.gray(
          "ℹ️  Please specify a repository URL or a local directory path."
        )
      );
      console.log();
